...
namespace AAV.CarChase.Scripts.Static {
  ...
  public static class GizmosExtra {
    ...
    public static void DrawWireCylinder(Vector3 pos, Vector3 dir, float radius, float height) {
      ...
      var quat = Quaternion.LookRotation(dir, new Vector3(-dir.y, dir.x, 0));

      Gizmos.DrawLine(pos + quat * new Vector3(radius, 0, halfHeight),
                      pos + quat * new Vector3(radius, 0, -halfHeight));
      Gizmos.DrawLine(pos + quat * new Vector3(-radius, 0, halfHeight),
                      pos + quat * new Vector3(-radius, 0, -halfHeight));
      Gizmos.DrawLine(pos + quat * new Vector3(0, radius, halfHeight),
                      pos + quat * new Vector3(0, radius, -halfHeight));
      Gizmos.DrawLine(pos + quat * new Vector3(0, -radius, halfHeight),
                      pos + quat * new Vector3(0, -radius, -halfHeight));
      ...
      for (float i = 0; i < 6.28f; i += 0.1f) {
        ...
        circle0Point1 =
            pos + quat * new Vector3(Mathf.Sin(i + 0.1f) * radius, Mathf.Cos(i + 0.1f) * radius, halfHeight);
